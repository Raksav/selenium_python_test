...
class Xpath_Util:
    ...
    def write_session(self, session_url, session_id):
        session_file = open("session.py", "w")
        text = ['session_url = "' + session_url + '"\n session_id = "' + session_id + '"']
        session_file.writelines(text)
        session_file.close()
...
if __name__ == "__main__":
    ...
    if not config.debug:
        ...
        session_file.writelines(text)
        session_file.close()
        print("session _url : " + session_url)
    ...
    if xpath_obj.generate_xpath(soup) is False:
        print("No XPaths generated for the URL:")
